...
import { Search, Trophy, ShoppingBag, X, SlidersHorizontal, ArrowUpDown, Sparkles, ShieldAlert, Award, Layers, HelpCircle, ChevronDown, ChevronUp, Info } from 'lucide-react';
        ...
        {processedItems.map(item => {
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-4 z-10">
                  ...
                  <img 
                    src={
                      item.image === 'default_item.png'
                        ? 'https://ddragon.leagueoflegends.com/cdn/14.8.1/img/item/1055.png' // ドランブレードをデフォルトのプレースホルダーにする
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-1">
                    <span className="text-[10px] font-black text-amber-500 bg-amber-500/10 border border-amber-500/20 rounded-md px-1.5 py-0.5">
                      {item.gold} G
                    </span>
                    {isOrnn && (
                      <span className="text-[9px] font-black text-amber-400 bg-amber-400/20 border border-amber-400/30 rounded-md px-1 py-0.5 flex items-center gap-0.5">
                        <Award size={10} />
                        傑作
                      </span>
                    )}
              ...
              {/* Stats overview */}
              <div className="space-y-1.5 mb-4 flex-1 z-10">
                {item.stats.length > 0 ? (
                  item.stats.slice(0, 3).map((stat, idx) => (
                    <div key={idx} className="text-xs text-slate-300 font-bold flex items-center gap-1.5 bg-slate-950/40 px-2.5 py-1 rounded-xl border border-slate-800/30">
                      <span>{getStatEmoji(stat)}</span>
                      <span className="truncate">{stat}</span>
                    </div>
                  ))
                ) : (
                  <div className="text-xs text-slate-500 italic font-semibold">効果のみ</div>
                )}
                {item.stats.length > 3 && (
                  <div className="text-[10px] text-indigo-400 font-black pl-7">
              ...
              {item.passives.length > 0 && (
                <div className="mb-4 z-10">
                  <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-1">
                    ...
                    パッシブ効果 ({item.passives.length})
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {item.passives.map((p, idx) => (
                      <span key={idx} className="text-[10px] font-bold bg-slate-800 text-slate-200 border border-slate-700/50 rounded-lg px-2 py-0.5">
                        {p.name}
                      </span>
                    ))}
                  </div>
